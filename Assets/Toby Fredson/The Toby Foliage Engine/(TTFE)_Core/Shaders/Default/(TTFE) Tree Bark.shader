	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float3 appendResult939_g246 = (float3(0.0 , 0.0 , saturate( v.positionOS.xyz ).z));
				float3 break989_g246 = v.positionOS.xyz;
				float3 appendResult938_g246 = (float3(break989_g246.x , ( break989_g246.y * 0.15 ) , 0.0));
				float mulTime975_g246 = _TimeParameters.x * 2.1;
				float3 temp_output_624_0_g246 = ( ( v.positionOS.xyz - float3(0,-1,0) ) / _Radius );
				...
				float2 appendResult928_g246 = (float2(v.positionOS.xyz.x , v.positionOS.xyz.z));
				float3 temp_output_996_0_g246 = ( cross( temp_cast_0 , float3( appendResult928_g246 ,  0.0 ) ) * 0.005 );
				float3 appendResult931_g246 = (float3(0.0 , v.positionOS.xyz.y , 0.0));
				float3 break971_g246 = v.positionOS.xyz;
				float3 appendResult967_g246 = (float3(break971_g246.x , 0.0 , ( break971_g246.z * 0.15 )));
				float mulTime976_g246 = _TimeParameters.x * 2.3;
				float dotResult849_g246 = dot( (v.positionOS.xyz*0.02 + 0.0) , v.positionOS.xyz );
				float CenterOfMassThicknessMask854_g246 = saturate( dotResult849_g246 );
				float3 appendResult981_g246 = (float3(v.positionOS.xyz.x , 0.0 , 0.0));
				float3 break984_g246 = v.positionOS.xyz;
				...
				float3 normalizeResult765_g246 = normalize( ase_worldPos );
				...
				float3 normalizeResult1092_g246 = normalize( v.positionOS.xyz );
				float CenterOfMassTrunkUP_C1098_g246 = saturate( distance( normalizeResult1092_g246 , float3(0,1,0) ) );
				float3 normalizeResult774_g246 = normalize( ase_worldPos );
				...
				float3 worldToObj907_g246 = mul( GetWorldToObjectMatrix(), float4( v.positionOS.xyz, 1 ) ).xyz;
				...
				float3 normalizeResult632_g246 = normalize( v.positionOS.xyz );
				...
				float3 worldToObj908_g246 = mul( GetWorldToObjectMatrix(), float4( v.positionOS.xyz, 1 ) ).xyz;
				...
				float3 normalizeResult697_g246 = normalize( v.positionOS.xyz );
				...
				float3 worldToObj1131_g246 = mul( GetWorldToObjectMatrix(), float4( v.positionOS.xyz, 1 ) ).xyz;
				float mulTime1138_g246 = _TimeParameters.x * 4.0;
				float mulTime1129_g246 = _TimeParameters.x * 0.2;
				float2 appendResult1126_g246 = (float2(ase_worldPos.x , ase_worldPos.z));
						 ...
						 ) : SV_Target
			{
				...
				float2 texCoord18_g247 = IN.ase_texcoord8.xy * _Tiling + _Offset;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 texCoord18_g247 = IN.ase_texcoord4.xy * _Tiling + _Offset;
				float CustomDRAWERS37_g247 = ( _TEXTUREMAPS + _TEXTURESETTINGS + _DIVIDER_01 + _DIVIDER_02 );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 texCoord18_g247 = IN.ase_texcoord2.xy * _Tiling + _Offset;
				float CustomDRAWERS37_g247 = ( _TEXTUREMAPS + _TEXTURESETTINGS + _DIVIDER_01 + _DIVIDER_02 );
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 texCoord18_g247 = IN.ase_texcoord5.xy * _Tiling + _Offset;
				float3 unpack7_g247 = UnpackNormalScale( tex2D( _NormalMap, texCoord18_g247 ), _NormalIntensity );
				unpack7_g247.z = lerp( 1, unpack7_g247.z, saturate(_NormalIntensity) );
...
ASEEND*/
//CHKSM=09FFC4BB1D5F62BD1EC450A70D99359134112983